...
def main(search_term, num_tweets):
    ...
    enriched_tweets = enrich(credentials,collection, sentiment_target = search_term)
    #
...
def store(tweets):
    for tweet in tweets:
        db_id = databaseInstance("alchemy").insert(tweet)

    db_count = databaseInstance("alchemy").count()

    print "Tweets stored in MongoDB! Number of documents in alchemy db: %d" % db_count
